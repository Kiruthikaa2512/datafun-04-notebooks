...
Author: Kiruthikaa NS - DataFun 04 - 2026-06
Date: 2026-04
...
uv run python -m datafun.app_kiruthikaa
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    scatter_plt.set_title("Flipper Length vs Bill Length by Island")
    ...
    box_plt.set_title("Flipper Length by Island")
...
def summarize(df: pd.DataFrame, df_clean: pd.DataFrame) -> None:
    ...
    LOG.info("  Grouping by island shows how location changes the EDA perspective.")
    LOG.info("Suggested next step: ")
    LOG.info("  Compare body mass and flipper length by island and species.")
